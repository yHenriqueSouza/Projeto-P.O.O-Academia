...
class Menu:
    """Menu interativo para gerenciar objetos da academia em tempo real."""
    def __init__(self):
        dados = a.carregar_dados_academia()
        
        if dados is not None:
            # Reconstrói os estados a partir da leitura limpa dos arquivos CSV
            self.rede = dados[0]
            self.filial = dados[1]
            ...
            # Aloca as referências padrões de navegação no menu usando o primeiro item da lista
            ...
            # Caso contrário, instancia os dados padrões do sistema em tempo de execução
            print("Carregando banco padrão inicial...")
            self.gerar_dados_padrao()

    def gerar_dados_padrao(self):
        # 1. Criação dos Exercícios
        supino = Exercicio()
        supino._init_("Supino Reto", "Peitoral", 4, 10, 40, 90)
        agachamento = Exercicio()
        agachamento._init_("Agachamento", "Pernas", 4, 12, 60, 120)
        remada = Exercicio()
        remada._init_("Remada Curvada", "Costas", 4, 10, 45, 90)
        ...
        treinoA = Treino()
        treinoA._init_("Treino A", "Hipertrofia", "Segunda e Quinta")
        treinoB = Treino()
        treinoB._init_("Treino B", "Hipertrofia", "Terça e Sexta")
        ...
        # 3. Gerente e Personal
        self.gerente = Gerente()
        self.gerente._init_("Marcos", 40, 85, 1.78, "Masculino", "Nenhuma", "(84)98888-0000", 7000.0, None)
        
        self.personal = Personal()
        self.personal._init_("Carlos", 30, 82, 1.80, "Masculino", "Nenhuma", "(84)99999-0000", 4500.0, "Musculação", None)

        # 4. Criação da Filial e da Rede
        self.filial = Filial()
        self.filial._init_(1, "Academia Central", "Rua Principal, 100", "(84)3333-3333", self.gerente, 300)
        self.gerente.filial = self.filial
        self.personal.filial = self.filial

        # 5. Criação de Alunos
        aluno1 = Aluno()
        aluno1._init_("Felipe", 18, 70, 1.75, "Masculino", "Nenhuma", "(84)99999-1111", "2025001", "Premium", "Hipertrofia", self.filial)
        aluno2 = Aluno()
        aluno2._init_("João", 22, 82, 1.81, "Masculino", "Nenhuma", "(84)99999-2222", "2025002", "Gold", "Emagrecimento", self.filial)
        ...
        self.rede = RedeAcademia()
        self.rede._init_("Academia PowerFit", "00.000.000/0001-00")
        self.rede.adicionar_filial(self.filial)

        # Grava os arquivos locais na primeira execução
